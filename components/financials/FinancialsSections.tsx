      ...
      <div className="flex gap-1 overflow-x-auto p-2 scrollbar-hide">
        {tabs.map((tab) => (
          <button
            ...
            className={`min-h-[44px] flex-shrink-0 rounded-xl px-4 py-3 text-xs font-black uppercase tracking-widest transition-all ${
    ...
    <div className="hidden border-b border-zinc-100 bg-zinc-50/50 p-2 sm:flex">
      {tabs.map((tab) => (
        <button
          ...
          type="button"
          onClick={() => onChange(tab)}
          className={`min-h-[44px] flex-1 rounded-xl px-4 py-3 text-xs font-black uppercase tracking-widest transition-all ${
    ...
    <>
      <div className="flex flex-col items-start gap-2 border-b border-zinc-100 px-4 py-3 sm:flex-row sm:items-center sm:gap-3 sm:px-8">
        ...
        <input
          ...
          className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-green-400 sm:w-52 sm:py-1.5"
        />
        {batchFilter ? (
        ...
        <tbody className="divide-y divide-zinc-100">
          {filteredInvoices.map((invoice) => (
            <tr key={invoice.id} className="transition-colors hover:bg-zinc-50">
              ...
              <td className="px-8 py-4">
                <span className="rounded-md bg-emerald-100 px-2 py-0.5 text-xs font-black uppercase tracking-tighter text-emerald-700">{invoice.status}</span>
              </td>
      ...
      {payments.map((payment) => (
        <div key={payment.id} className="rounded-xl border border-zinc-100 bg-white p-4 shadow-sm">
          <div className="mb-2 flex items-start justify-between">
            ...
            <span className="rounded-md bg-green-100 px-2 py-0.5 text-xs font-black uppercase tracking-tighter text-green-700">{payment.type}</span>
          </div>
...
export const ReceiptsSection: React.FC<ReceiptsSectionProps> = ({
  ...
  <div className="p-3 sm:p-8">
    {receipts.length === 0 ? (
      <div className="mx-auto max-w-lg rounded-2xl bg-green-50 p-6 text-center sm:p-8">
        ...
        <button type="button" onClick={onRecordPayment} className="rounded-xl bg-green-600 px-6 py-3 font-bold text-white hover:bg-green-700 touch-manipulation">
          Record Payment
        </button>
      </div>
    ) : (
      <>
        <div className="mb-4 flex flex-col items-start justify-between gap-3 sm:mb-6 sm:flex-row sm:items-center">
          ...
          <button type="button" onClick={onRecordPayment} className="w-full rounded-xl bg-green-600 px-6 py-3 font-bold text-white hover:bg-green-700 touch-manipulation sm:w-auto">
            Record Payment
          </button>
        </div>
        <div className="space-y-3 sm:hidden">
          {receipts.map((receipt) => (
            <div key={receipt.id} className="rounded-xl border border-zinc-100 bg-white p-4 shadow-sm">
              ...
              {receipt.batch ? (
                <span className="mb-3 inline-block rounded-md bg-blue-50 px-2 py-0.5 font-mono text-[11px] font-bold text-blue-700">{receipt.batch}</span>
              ) : null}
    ...
    <div className="mx-auto max-w-lg rounded-2xl bg-blue-50 p-8 text-center">
      ...
      <div className="mb-3 grid gap-3 sm:grid-cols-2">
        <div className="text-left">
          ...
          <input type="date" value={statementDateFrom} onChange={(event) => onDateFromChange(event.target.value)} className="w-full rounded-xl border border-zinc-200 px-4 py-3" />
        </div>
        <div className="text-left">
          <label className="mb-1 block text-xs font-black uppercase tracking-widest text-zinc-500">To</label>
          <input type="date" value={statementDateTo} onChange={(event) => onDateToChange(event.target.value)} className="w-full rounded-xl border border-zinc-200 px-4 py-3" />
        </div>
      </div>
      <div className="flex flex-col justify-center gap-3 sm:flex-row">
        <select value={selectedClient} onChange={(event) => onClientChange(event.target.value)} className="rounded-xl border border-zinc-200 px-4 py-3">
        ...
        <button type="button" onClick={onGenerate} className="rounded-xl bg-blue-600 px-6 py-3 font-bold text-white hover:bg-blue-700">
          Generate
        </button>
        <button
          type="button"
