    ...
    <tr className="hover:bg-zinc-50 transition-all group">
      ...
      <td className="px-8 py-6">
        <div className="flex flex-col">
          <span className="font-black text-base text-zinc-900">{vehicle.make_model}</span>
          <span className="font-mono text-xs font-bold uppercase tracking-[0.08em] text-zinc-600">
            {vehicle.vin_number}
          </span>
        </div>
      </td>
      ...
      <td className="px-8 py-6 font-bold tracking-tight tabular-nums text-zinc-600">
      ...
      <td className="px-8 py-6">
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <span className="font-black text-lg tabular-nums text-zinc-900">
...
export const VehicleList: React.FC<VehicleListProps> = memo(({ vehicles, onEdit, onDelete }) => {
  ...
  if (vehicles.length === 0) {
    return (
      <div className="p-12 text-center" style={{ background: '#ffffff', border: '1px solid #e7e5e4' }}>
        <svg
        ...
        <h3 className="text-lg font-bold mb-2 text-zinc-900">No Vehicles Found</h3>
        <p style={{ color: '#52525b' }}>Add your first vehicle to start tracking your fleet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden" style={{ background: '#ffffff', border: '1px solid #e7e5e4' }}>
      ...
      <div className="px-8 py-6 border-b border-zinc-100 flex justify-between items-center bg-zinc-50/30">
        <div>
          <h3 className="text-xl font-black tracking-tight text-zinc-900">Current Inventory</h3>
          <p className="text-sm mt-1 text-zinc-600">
            {vehicles.length} vehicle{vehicles.length !== 1 ? 's' : ''} • {inTransitCount} in transit
          </p>
        </div>
        <div className="text-right">
          <p className="text-xs font-bold uppercase tracking-widest text-zinc-600">Total Value</p>
