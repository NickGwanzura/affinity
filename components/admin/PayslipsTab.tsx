        ...
        <div className="space-y-3 p-3 sm:hidden">
          {payslips.length === 0 ? (
            ...
            payslips.map((payslip) => (
              <div key={payslip.id} className="p-4" style={{ border: '1px solid var(--cds-border-subtle, #e7e5e4)', background: 'var(--cds-background, #ffffff)' }}>
                <div className="mb-2 flex items-start justify-between">
                  <div>
                    <div className="font-bold" style={{ color: 'var(--cds-text-primary, #18181b)' }}>{payslip.employee?.name || 'N/A'}</div>
                    <div className="font-mono text-xs" style={{ color: 'var(--cds-text-secondary, #52525b)' }}>{payslip.payslip_number}</div>
                  </div>
                  <span className="inline-block px-2 py-0.5 text-xs font-semibold" style={
                    payslip.status === 'Generated' ? { background: 'var(--cds-support-info-inverse, #dbeafe)', color: 'var(--cds-interactive, #D97706)' } :
                    payslip.status === 'Approved' ? { background: 'var(--cds-support-warning-inverse, #fdf6dd)', color: 'var(--cds-support-warning-inverse, #92400e)' } :
                    payslip.status === 'Paid' ? { background: 'var(--cds-support-success-inverse, #d1fae5)', color: 'var(--cds-support-success, #10b981)' } :
                    { background: 'var(--cds-support-error-inverse, #fee2e2)', color: 'var(--cds-support-error, #dc2626)' }
                  }>
                    {payslip.status}
                  </span>
                </div>
                <div className="mb-1 text-xs" style={{ color: 'var(--cds-text-secondary, #52525b)' }}>{new Date(payslip.year, payslip.month - 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</div>
                <div className="mb-3 text-sm">
                  <span style={{ color: 'var(--cds-text-secondary, #52525b)' }}>${payslip.gross_pay.toLocaleString()} gross</span>
                  <span className="mx-1" style={{ color: 'var(--cds-text-secondary, #52525b)' }}>&rarr;</span>
                  <span className="font-bold" style={{ color: 'var(--cds-support-success, #10b981)' }}>${payslip.net_pay.toLocaleString()} net</span>
                </div>
                <div className="flex flex-wrap gap-2 border-t border-zinc-50 pt-3">
                  {payslip.status === 'Generated' && (
                    <button onClick={() => handleUpdatePayslipStatus(payslip.id, 'Approved')} className="px-2 py-1 text-xs font-bold" style={{ color: 'var(--cds-support-warning-inverse, #92400e)' }}>Approve</button>
                  )}
                  {payslip.status === 'Approved' && (
                    <button onClick={() => handleUpdatePayslipStatus(payslip.id, 'Paid')} className="px-2 py-1 text-xs font-bold" style={{ color: 'var(--cds-support-success, #10b981)' }}>Mark Paid</button>
                  )}
                  <button
                  ...
                  <button onClick={() => handleDeletePayslip(payslip.id)} className="px-2 py-1 text-xs font-bold" style={{ color: 'var(--cds-support-error, #dc2626)' }}>Delete</button>
                </div>
              </div>
            ))
          )}
        ...
        <div className="hidden sm:block overflow-x-auto">
          <table className="w-full">
            <thead className="bg-zinc-50 border-b border-zinc-200">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--cds-text-secondary, #52525b)' }}>Payslip #</th>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--cds-text-secondary, #52525b)' }}>Period</th>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--cds-text-secondary, #52525b)' }}>Gross Pay</th>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--cds-text-secondary, #52525b)' }}>Net Pay</th>
              ...
              {payslips.length === 0 ? (
                ...
                payslips.map((payslip) => (
                  <tr key={payslip.id} className="hover:bg-zinc-50">
                    <td className="px-6 py-4 font-mono text-sm" style={{ color: 'var(--cds-text-secondary, #52525b)' }}>{payslip.payslip_number}</td>
                    <td className="px-6 py-4 font-semibold" style={{ color: 'var(--cds-text-primary, #18181b)' }}>{payslip.employee?.name || 'N/A'}</td>
                    <td className="px-6 py-4" style={{ color: 'var(--cds-text-secondary, #52525b)' }}>{new Date(payslip.year, payslip.month - 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</td>
                    <td className="px-6 py-4 font-semibold" style={{ color: 'var(--cds-text-primary, #18181b)' }}>${payslip.gross_pay.toLocaleString()}</td>
                    <td className="px-6 py-4 font-bold" style={{ color: 'var(--cds-support-success, #10b981)' }}>${payslip.net_pay.toLocaleString()}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {payslip.status === 'Generated' && (
                          <button onClick={() => handleUpdatePayslipStatus(payslip.id, 'Approved')} className="font-semibold text-sm" style={{ color: 'var(--cds-support-warning-inverse, #92400e)' }}>Approve</button>
                        )}
                        {payslip.status === 'Approved' && (
                          <button onClick={() => handleUpdatePayslipStatus(payslip.id, 'Paid')} className="font-semibold text-sm" style={{ color: 'var(--cds-support-success, #10b981)' }}>Mark Paid</button>
                        )}
                        <button
                          onClick={() => handleDownloadPayslip(payslip)}
                        ...
                        <button onClick={() => handleDeletePayslip(payslip.id)} className="font-semibold text-sm" style={{ color: 'var(--cds-support-error, #dc2626)' }}>Delete</button>
                      </div>
                    </td>
                  </tr>
                ))
