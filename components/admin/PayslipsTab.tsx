  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <p className="font-bold animate-pulse uppercase tracking-widest text-xs text-zinc-600">Loading Payslips</p>
      </div>
    );
  }
        ...
        <div className="space-y-3 p-3 sm:hidden">
          {payslips.length === 0 ? (
            ...
            payslips.map((payslip) => (
              <div key={payslip.id} className="p-4" style={{ border: '1px solid #e7e5e4', background: '#ffffff' }}>
                <div className="mb-2 flex items-start justify-between">
                  <div>
                    <div className="font-bold text-zinc-900">{payslip.employee?.name || 'N/A'}</div>
                    <div className="font-mono text-xs text-zinc-600">{payslip.payslip_number}</div>
                  </div>
                  <span className="inline-block px-2 py-0.5 text-xs font-semibold" style={
                    payslip.status === 'Generated' ? { background: '#dbeafe', color: '#D97706' } :
                    payslip.status === 'Approved' ? { background: '#fdf6dd', color: '#92400e' } :
                    payslip.status === 'Paid' ? { background: '#d1fae5', color: '#10b981' } :
                    { background: '#fee2e2', color: '#dc2626' }
                  }>
                    {payslip.status}
                  </span>
                </div>
                <div className="mb-1 text-xs text-zinc-600">{new Date(payslip.year, payslip.month - 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</div>
                <div className="mb-3 text-sm">
                  <span style={{ color: '#52525b' }}>${payslip.gross_pay.toLocaleString()} gross</span>
                  <span className="mx-1 text-zinc-600">&rarr;</span>
                  <span className="font-bold text-emerald-500">${payslip.net_pay.toLocaleString()} net</span>
                </div>
                <div className="flex flex-wrap gap-2 border-t border-zinc-50 pt-3">
                  {payslip.status === 'Generated' && (
                    <button onClick={() => handleUpdatePayslipStatus(payslip.id, 'Approved')} className="px-2 py-1 text-xs font-bold text-amber-900">Approve</button>
                  )}
                  {payslip.status === 'Approved' && (
                    <button onClick={() => handleUpdatePayslipStatus(payslip.id, 'Paid')} className="px-2 py-1 text-xs font-bold text-emerald-500">Mark Paid</button>
                  )}
                  <button
                  ...
                  <button onClick={() => handleDeletePayslip(payslip.id)} className="px-2 py-1 text-xs font-bold text-red-600">Delete</button>
                </div>
              </div>
            ))
          )}
        ...
        <div className="hidden sm:block overflow-x-auto">
          <table className="w-full">
            <thead className="bg-zinc-50 border-b border-zinc-200">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-[0.08em] text-zinc-600">Payslip #</th>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-[0.08em] text-zinc-600">Employee</th>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-[0.08em] text-zinc-600">Period</th>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-[0.08em] text-zinc-600">Gross Pay</th>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-[0.08em] text-zinc-600">Net Pay</th>
                <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-[0.08em] text-zinc-600">Status</th>
              ...
              {payslips.length === 0 ? (
                ...
                payslips.map((payslip) => (
                  <tr key={payslip.id} className="hover:bg-zinc-50">
                    ...
                    <td className="px-6 py-4 font-semibold text-zinc-900">${payslip.gross_pay.toLocaleString()}</td>
                    <td className="px-6 py-4 font-bold text-emerald-500">${payslip.net_pay.toLocaleString()}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {payslip.status === 'Generated' && (
                          <button onClick={() => handleUpdatePayslipStatus(payslip.id, 'Approved')} className="font-semibold text-sm text-amber-900">Approve</button>
                        )}
                        {payslip.status === 'Approved' && (
                          <button onClick={() => handleUpdatePayslipStatus(payslip.id, 'Paid')} className="font-semibold text-sm text-emerald-500">Mark Paid</button>
                        )}
                        <button
                          onClick={() => handleDownloadPayslip(payslip)}
                        ...
                        <button onClick={() => handleDeletePayslip(payslip.id)} className="font-semibold text-sm text-red-600">Delete</button>
                      </div>
                    </td>
                  </tr>
                ))
